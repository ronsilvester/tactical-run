...
class Workout:
    def __init__(self, date: datetime, distance: float, rpe: int, duration_min:int):
    ...
    def __str__(self):
        return f"Workout on {self.date.strftime('%Y-%m-%d %H:%M:%S')}:{self.duration_min} mins, {self.distance} km, RPE {self.rpe}"
...
class User:
    def __init__(self, name: str, age: int, weight: float, height: float, gender: Gender):
        self.name = name
        self.age = age
        self.weight = weight
        self.height = height
    ...
    def add_workout(self, workout: Workout):
        if not isinstance(workout, Workout):
            raise ValueError("Expected a Workout instance")
